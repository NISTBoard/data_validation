...
def main():
    ...
    comments = open('CDS_Validation_Comments', 'w')
...
def populate_overlay_dict(sheet, inp_dict):
    titles = ['CROSS DOMAIN SOLUTIONS', 'TRANSFER', 'ACCESS', 'MULTILEVEL', 'CROS', 'S DOMAIN SOLUTIONS']
    for i in range(29, 174):
        if not sheet.cell(row=i, column=1).value in titles:
            ...
            inp_dict[sheet.cell(row=i, column=1).value] = [sheet.cell(row=i, column=3).value]
            inp_dict[sheet.cell(row=i, column=1).value].append(sheet.cell(row=i, column=6).value)
...
def populate_data_dict(worksheet, inp):
    for i in range(4, worksheet.max_row + 1):
        inp[worksheet.cell(row=i, column=3).value] = [worksheet.cell(row=i, column=53).value]
        inp[worksheet.cell(row=i, column=3).value].append(worksheet.cell(row=i, column=51).value)
        inp[worksheet.cell(row=i, column=3).value].append(worksheet.cell(row=i, column=52).value)
    #print("Data Dict: ", inp)
...
def compare_dicts(data, overlay, outfile):
    ...
    if switch == 0:
        ...
        print("\n" + "There were some discrepancies. Check 'CDS_Validation_Comments' for more information.")
